...
CREATE TABLE `player_profile` (
  `playerId` bigint(20) NOT NULL DEFAULT 10000,
  `pot` int(11) NOT NULL,
...
CREATE TABLE `user` (
  ...
  PRIMARY KEY (`id`),  
  CONSTRAINT `FK_player_id` FOREIGN KEY (`FK_player_id`) REFERENCES `player_profile` (`playerd`) ON DELETE CASCADE ON UPDATE CASCADE
